  ...
  projects: [
    {
      ...
      image: 'images/finbert-signals.jpg'
    },
    {
      ...
      image: 'images/clv-segmentation.jpg'
    },
    {
      ...
      image: 'images/budget-optimizer.jpg'
    },
    {
      ...
      image: 'images/funnel-abtest.jpg'
    }
  ],
...
// Experience (keep all bullets; Show more beyond 4)
const expEl = document.getElementById('expTimeline');
if (expEl){
  data.experiences.forEach(x=>{
    const r = document.createElement('div');
    r.className='role reveal';
    r.innerHTML = `
      <h3>${x.role} — ${x.company}</h3>
      <p class="meta">${x.location} · ${x.period}</p>
      <ul class="role-bullets">${(x.bullets||[]).map(b=>`<li>${b}</li>`).join('')}</ul>
    `;
    expEl.appendChild(r);

    const ul = r.querySelector('.role-bullets');
    const items = Array.from(ul.children);
    const threshold = 4;
    if (items.length > threshold){
      items.slice(threshold).forEach(li=>li.classList.add('hidden'));
      ...
      btn.addEventListener('click', ()=>{
        ...
        items.slice(threshold).forEach(li=>li.classList.toggle('hidden', expanded));
